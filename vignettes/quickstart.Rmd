...
```{r, results='hide'}
gpmc <- gp_mcmc(gp, x, y, trials=rep(1,n), iter=1000, chains=2)
```
...
As we see, the fit is substantially different in those cases where the predictive probability is close to zero or one. It is worthwhile to note that even though the Laplace approximation is not very accurate in the extreme cases, it gives reasonable estimates for the hyperparameters. Running the MCMC for the latent values given the hyperparameters gives then accurate estimates for the latent function values but is much more efficient than sampling the hyperparameters and the latent values jointly. In this case the MCMC takes only a few seconds.
