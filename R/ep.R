...
ep_iter.approx_full <- function(object, gp, K, y, fmean_old, fvar_old, z_old, P_old,
  ...
  if (all(V > 0)) {
    ...
    args <- c(
      list(object=object, gp=gp, K=K[good,good,drop=F], y=y[good], 
           fmean_old=NULL, fvar_old=NULL, z_old=NULL, P_old=NULL,
           pobs=list(z=z[good], var=V[good], log_normalizing_const=log_Z[good], 
    ...
    df2_invC2_df2 <- (pred_mean-z[bad]) %*% backsolve(t(Lk), backsolve(t(A_chol), 
